		...
		return this.getData()
			.then((results) => {
				matchedUsers = _.filter(results.usersList, (user) => {
					console.dir(`${userName} vs. ${user.username}`);
					return userName === user.username;
				});

				if (matchedUsers.length > 0) {
					let matchedUser = matchedUsers[0];
					console.dir(`~~~ Got user ${matchedUser.name} - is admin: ${matchedUser.isAdmin}`);

					if (matchedUser.isAdmin && text.toLowerCase() === 'refresh'){
						console.dir('refresh');
						bot.refreshSchedule();
					}
				}
	...
	scheduleChores() {
		...
		return this.getData()
			.then((results) => {
				//console.dir(results);
				_.forEach(results.choresList, (chore) => {
					console.dir(`scheduled ${chore.name}`);
